...
const CallItem = ({ call, indx, onOpenClick, onCloseClick, modalId }) => {
  ...
  return (
    <MotionComponent
      ...
      className=" w-full p-[2px] rounded-tl-[15px] rounded-r-[15px] rounded-bl-[15px] max-md:mb-1 mb-2 rounded-br-[15px]  rounded bg-gradient-to-br from-green-600 from-[10%] via-bgLightGreenTwo/90 via-[50%]  to-emerald-700  to-[90%]  shadow-lg drop-shadow-lg z-0 ">
      <div
        className={`relative w-full py-2 px-5 max-sm:px-3 max-sm:py-1 bg-white  dark:bg-blue rounded-tl-[22px] rounded-tr-[15px]  rounded-br-[22px] rounded-bl-[15px] flex justify-between items-center  group overflow-hidden hover:bg-slate-100 h-12 transition-all duration-200  cursor-pointer ${
          modalId === call.id && "max-md:h-18 h-20"
        }`}>
        <div
          ...
          <div>
            ...
            {modalId === call.id && (
              <MotionComponent transition={{ duration: 0.3 }}>
        ...
        <p className="text-sm text-gray-400 max-md:font-normal font-semibold max-lg:px-8 pr-0 ">
          {dayjs(call.created_at).format("h:mm A")}
        </p>

        <div className="absolute right-0 max-lg:right-9 top-0 h-full flex items-center transition-transform duration-300 ease-in-out transform translate-x-full group-hover:translate-x-0 max-lg:group-hover:translate-x-10 cursor-pointer z-10">
          <div
            id="archive"
