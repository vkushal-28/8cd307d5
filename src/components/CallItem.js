...
const CallItem = ({ call, indx, onOpenClick, onCloseClick, modalId }) => {
  ...
  return (
    <motion.div
      ...
      className=" w-full p-[2px] rounded-tl-[15px] rounded-r-[15px] rounded-bl-[15px] max-md:mb-1.5 mb-2 rounded-br-[15px]  rounded bg-gradient-to-br from-green-600 from-[10%] via-bgLightGreenTwo/90 via-[50%]  to-emerald-700  to-[90%]  shadow-lg drop-shadow-lg z-0 cursor-pointer">
      <div
        className={`relative w-full py-2 px-5 max-sm:px-3 max-sm:py-1 bg-white  dark:bg-blue rounded-tl-[22px] rounded-tr-[15px]  rounded-br-[22px] rounded-bl-[15px] flex justify-between items-center  group overflow-hidden hover:bg-slate-100 h-14 transition-all duration-200 ${
          modalId === call.id && "h-20"
        }`}>
        <div className="flex justify-start items-center w-[90%] max-lg:w-[60%] max-md:w-[60%] z-20 ">
          <i
            className={`fi ${
          ...
          <div>
            ...
            <p className="text-sm line-clamp-1 text-slate-400  ">
              {`called to ${call.to}  via ${call.via}`}
            </p>
            {modalId === call.id && (
              <>
                <div className="text-slate-500 text-sm">
                  <p>
                    ...
                    {seconds_to_days_hours_mins_secs_str(call.duration)}
                  </p>
                </div>
                {/* <p className="text-sm">{!call.is_archived && "Archived"}</p> */}
              </>
            )}
          </div>
        </div>
